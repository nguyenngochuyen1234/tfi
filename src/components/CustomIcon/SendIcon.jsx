import React from "react";
import PropTypes from "prop-types";
import Icon from "@ant-design/icons/lib/components/Icon";
import classNames from "classnames";
import "./style.css";
SendIcon.propTypes = {
    width: PropTypes.string,
    height: PropTypes.string,
    active: PropTypes.bool,
    handleSend: PropTypes.func.isRequired
};
SendIcon.defaultProps = {
    width:"24px",
    height:"24px",
    active: false,
};

function SendIcon({width,height,active,handleSend}) {
    const SendSgv = () => (
        <svg width={width} height={height} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
                d="M9.51002 4.23001L18.07 8.51001C21.91 10.43 21.91 13.57 18.07 15.49L9.51002 19.77C3.75002 22.65 1.40002 20.29 4.28002 14.54L5.15002 12.81C5.37002 12.37 5.37002 11.64 5.15002 11.2L4.28002 9.46001C1.40002 3.71001 3.76002 1.35001 9.51002 4.23001Z"
                stroke="#000"
                strokeLinecap="round"
                strokeLinejoin="round"
            />
            <path d="M5.44 12H10.84" stroke="#000" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
    );
    return <Icon onClick={handleSend} className={classNames({"active":active,"df":true,"icon-cursor":true})} component={SendSgv} />;
}
export default SendIcon;
